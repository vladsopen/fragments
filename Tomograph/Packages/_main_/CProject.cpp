...
#define VERIFY_WHOLE_PROGRAM_OPTIMIZATION VERIFY_PROJECT(Tomo)
#include "CProject.h"
//(after-main-include-in-main-cpp-Tomo)
...
void TheProjectCallback(out project_definition& out_oDefine)
{
    ...
    out_oDefine.m_szProjectLogo = "Tomo";
    DEFINE_PROJECT_ATTRIBUTES();

    out_oDefine.m_fAppOption |= 0 |
        //0 //project_definition::F_AppOption_UniViewFromRegistryOnly : 0)

        // we have essential config
...
void CProject::OnInitProject()
{
    ...
    // Init for PatientEmbeded mode
    x_rPatientEmbedLink->
        InitPatientEmbedLink(
            ref<CPatientEmbedConnectionIfaceGp>());
}
...
void CProject::OnHandleAppActivationStateChange(
        EActivationStateChange eActivationStateChange)
{

    if (x_rLoader->IsActiveSessionPresent())
    {
        x_rLoader->
            x_rSession->
                x_rDeviceManager->
                    HandleAppActivationStateChange(
                        eActivationStateChange);
    }

    if (eActivationStateChange == E_ActivationStateChange_AppActivated)
    {
    }
}
